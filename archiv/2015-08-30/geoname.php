<?php
	...
	{
		// cols: source, type, name, frequency, callsign, latitude, longitude, elevation
	
		$query = "(SELECT 'openaip_airports' AS source, type, CONCAT(icao, ' (', name ,')') AS name, NULL AS frequency, NULL AS callsign, latitude, longitude, elevation FROM openaip_airports WHERE";
		...
		$query .= "(SELECT 'openaip_navaids' AS source, type, CONCAT(name, ' (', type, ')') AS name, frequency, kuerzel AS callsign, latitude, longitude, elevation FROM openaip_navaids WHERE";
		...
		$query .= "(SELECT 'geonames' AS source, NULL as type, name, NULL AS frequency, NULL AS callsign, latitude, longitude, elevation FROM geonames WHERE";
	...
	function getCoordinateSearchQuery($lat, $lon, $rad)
	{
		...
		$query .= "SELECT 1 AS sortOrder, 'openaip_airports' AS source, type, CONCAT(icao, ' (', name ,')') AS name, NULL AS frequency, NULL AS callsign, latitude, longitude, elevation FROM openaip_airports WHERE";
		...
		$query .= "SELECT 1 AS sortOrder, 'openaip_navaids' AS source, type, CONCAT(name, ' (', type, ')') AS name, frequency, kuerzel AS callsign, latitude, longitude, elevation FROM openaip_navaids WHERE";
		$query .= " latitude > " . ($lat - $rad);
		$query .= " AND latitude < " . ($lat + $rad);
		$query .= " AND longitude > " . ($lon - $rad);
		$query .= " AND longitude < " . ($lon + $rad);
		...
		$query .= "SELECT 3 AS sortOrder, 'geonames' AS source, NULL as type, name, NULL AS frequency, NULL AS callsign, latitude, longitude, elevation FROM geonames WHERE";
		$query .= " latitude > " . ($lat - $rad);
		$query .= " AND latitude < " . ($lat + $rad);
		$query .= " AND longitude > " . ($lon - $rad);
		$query .= " AND longitude < " . ($lon + $rad);
	...
	function buildReturnObject($result)
	{
		...
		{
/*			if ($rs["source"] == 'openaip_airports')
				$name = $rs["name"] . " (" . $rs["alternatenames"] . ")";
			elseif ($rs["source"] == 'openaip_navaids')
				$name = $rs["name"] . " (" . $rs["type"] . ")";
			else
				$name = $rs["name"];*/
