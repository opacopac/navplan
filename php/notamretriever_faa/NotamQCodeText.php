...
class NotamQCodeText
{
    /**
     * Q-Code Area definitions (first letter after Q)
     * Based on FAA NOTAM Appendix B - Table headers
     */
    ...
    /**
     * Q-Code SubArea definitions (first letter after Q)
     * Based on FAA NOTAM Appendix B - Table headers
     */
    private static array $subAreas = [
    ...
    /**
     * Q-Code Subject definitions (2 letters after Q)
     * Based on FAA NOTAM Appendix B - Significance column
     */
    private static array $subjects = [
        ...
        'FT' => 'Transmissometer of transmissometer)',
    ...
    /**
     * Q-Code Status definitions (last 2 letters)
     * Based on FAA NOTAM Appendix B - Significance column
     */
    ...
    /**
     * Get subject text from Q-Code
     * @param string $qCode Full Q-Code string (e.g., "QWELW") - extracts characters 2-3
     * @return string Subject text or empty string if not found
     */
    public static function getSubjectText(string $qCode): string
    {
        $qCode = strtoupper(trim($qCode));

        // Q-Code format: Q + 2-letter subject + 2-letter status = 5 characters minimum
        if (strlen($qCode) < 3 || $qCode[0] !== 'Q') {
            return '';
        }

        $subjectCode = substr($qCode, 1, 2);
        return self::$subjects[$subjectCode] ?? '';
    }


    /**
     * Get status text from Q-Code
     * @param string $qCode Full Q-Code string (e.g., "QWELW") - extracts characters 4-5
     * @return string Status text or empty string if not found
     */
    public static function getStatusText(string $qCode): string
    {
        $qCode = strtoupper(trim($qCode));

        // Q-Code format: Q + 2-letter subject + 2-letter status = 5 characters minimum
        if (strlen($qCode) < 5 || $qCode[0] !== 'Q') {
            return '';
        }

        $statusCode = substr($qCode, 3, 2);
        return self::$statuses[$statusCode] ?? '';
    }


    /**
     * Get area text from Q-Code
     * @param string $qCode Full Q-Code string (e.g., "QWELW") - extracts character 1 (first after Q)
     * @return string Area text or empty string if not found
     */
    public static function getAreaText(string $qCode): string
    {
        $qCode = strtoupper(trim($qCode));

        // Q-Code format: Q + area letter + ... = 2 characters minimum
        if (strlen($qCode) < 2 || $qCode[0] !== 'Q') {
            return '';
        }
    ...
    /**
     * Get sub-area text from Q-Code
     * @param string $qCode Full Q-Code string (e.g., "QWELW") - extracts character 1 (first after Q)
     * @return string Sub-area text or empty string if not found
     */
    public static function getSubAreaText(string $qCode): string
    {
        $qCode = strtoupper(trim($qCode));
